...
class DigitalTwinState:
    ...
    history: Dict[str, List] = field(
        default_factory=lambda: {"times": [], "errors": [], "predictions": [], "observations": []}
    )
...
class DigitalTwin:
    ...
    def __init__(self, config: DigitalTwinConfig):
        ...
        self.forecaster_type = (
            ForecasterType(config.forecaster_type)
            if hasattr(config, "forecaster_type")
            else ForecasterType.PERSISTENCE
        )
        ...
        if config.verbose:
            logger.info(
                f"Digital Twin initialized: {config.n_spatial_modes} modes, {config.n_sensors} sensors"
            )
    ...
    def train(
        ...
        self.decomposer = TuckerDecomposer(
            tensors=data_dict, device=self.config.device, config=decomp_config
        )
        ...
        qr_decomposer = TensorTubeQRDecomposition(tensor=A_tensor, config=sensor_config)
        ...
        self.sensor_indices = torch.nonzero(self.sensor_mask.reshape(-1), as_tuple=False).squeeze(
            -1
        )
    ...
    def _train_forecaster(
        self, data_dict: Dict[str, torch.Tensor], sample_tensor: torch.Tensor
    ) -> Dict[str, Any]:
    ...
    def _train_linear_forecaster(
        self, modal_history: torch.Tensor, config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _train_mlp_forecaster(
        self, modal_history: torch.Tensor, n_modes: int, config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _project_to_modal_space(self, state: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            if A_tensor.ndim == 2:
                ...
                x_modal = torch.linalg.lstsq(A_tensor, state_flat.unsqueeze(-1)).solution.squeeze(
                    -1
                )
            else:
    ...
    def _reconstruct_from_modal(self, modal_coeffs: torch.Tensor) -> torch.Tensor:
        ...
        if modal_coeffs.ndim == 1:
            reconstructed = reconstruct_tensor(
                A_tensor=A_tensor, x_hat=modal_coeffs, zero_threshold=1e-6, decimals=4
            )
            ...
            for t in range(n_steps):
                rec_t = reconstruct_tensor(
                    A_tensor=A_tensor, x_hat=modal_coeffs[:, t], zero_threshold=1e-6, decimals=4
                )
        ...
        if self.proxy_model is not None:
            ...
            if is_reservoir_state:
                if current_state.saturation is not None:
                    ...
                    state_tensor = torch.stack(
                        [current_state.pressure, current_state.saturation], dim=-1
                    )
                else:
            ...
            if is_reservoir_state:
                ...
                if current_state.saturation is not None:
                    # Split back
                    return [
                        ReservoirState(pressure=prediction[..., 0], saturation=prediction[..., 1])
                    ]
                else:
                    # Keep combined
    ...
    def sensor_locations(self) -> torch.Tensor:
        ...
        if self.sensor_indices is None:
            spatial_shape = (
                self._spatial_shape if self._spatial_shape else self.config.spatial_shape
            )
    ...
    def predict(
        ...
        modal_forecast = self._forecast_modal_coefficients(
            modal_current, n_steps, use_history, external_history=modal_history_tensor
        )
        ...
        if forecast.ndim > current_state.ndim:
            ...
            forecast = (
                forecast.unsqueeze(-1).repeat(1, 1, n_steps)
                if current_state.ndim == 2
                else forecast.unsqueeze(-1)
            )
    ...
    def _forecast_modal_coefficients(
        ...
        try:
            if self.forecaster_type == ForecasterType.LINEAR:
                ...
                seq_length = (
                    self.forecaster.seq_length if hasattr(self.forecaster, "seq_length") else 5
                )
            ...
            modal_forecast = torch.tensor(
                future_seq, device=self.device, dtype=self.dtype
    ...
    def _prepare_sensor_measurements(self, sensor_readings: torch.Tensor) -> torch.Tensor:
        ...
        if readings.shape[0] == n_sensors:
            ...
            full_flat = torch.zeros(
                (flat_mask.numel(),) + trailing, device=self.device, dtype=self.dtype
            )
    ...
    def update_from_sensors(
        self, sensor_readings: torch.Tensor, timestamp: Optional[float] = None
    ) -> torch.Tensor:
        ...
        for idx, Y_slice in enumerate(Y_slices):
            ...
            if self.config.verbose:
                logger.info(
                    f"CS Reconstruction slice {idx}: converged={metrics.converged}, "
                    f"iters={metrics.iterations}, obj={metrics.objective:.4e}"
                )

            X_reconstructed = reconstruct_tensor(
                A_tensor=A_tensor, x_hat=x_hat, zero_threshold=1e-4, decimals=3
            )
        ...
        if len(reconstructed_slices) == 1:
            ...
            reconstructed = torch.stack(reconstructed_slices, dim=-1).reshape(
                spatial_shape + trailing_shape
            )
        ...
        last_reconstructed = (
            reconstructed if reconstructed.ndim == len(spatial_shape) else reconstructed[..., -1]
        )
        if self.mean is not None:
        ...
        if self.proxy_model is not None:
            observed_state = self.create_reservoir_state(
                pressure=last_reconstructed, time=self.state.current_time
            )
            try:
    ...
    def evaluate_scenarios(
        self, scenarios: List[Dict[str, Any]], n_steps: int = 10
        ...
        for scenario in scenarios:
            ...
            if "initial_state" in scenario:
                ...
                initial_state = self._reconstruct_from_modal(self.state.modal_coefficients[:, 0])
            else:
            ...
            try:
                ...
                forecast = self.predict(
                    current_state=initial_state, n_steps=n_steps, return_full_field=True
                )
    ...
    def detect_anomalies(
        self, sensor_data: torch.Tensor, threshold: float = 3.0
    ) -> List[Dict[str, Any]]:
        ...
        spatial_ndim = (
            len(self._spatial_shape) if self._spatial_shape is not None else sensor_tensor.ndim
        )
        ...
        for t in range(n_timesteps):
            try:
                ...
                # Create solver.
                solver = TensorCompressiveSensing(A_tensor, P, Y, cs_config, ext_config)
                ...
                X_reconstructed = reconstruct_tensor(
                    A_tensor=A_tensor, x_hat=x_hat, zero_threshold=1e-6, decimals=4
                )
                ...
                anomalies.append({"timestamp": t, "error": str(e), "severity": "error"})
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "n_sensors": self.config.n_sensors,
            "modal_dim": int(self.spatial_modes.shape[-1])
            if self.spatial_modes is not None
            else None,
            "sensors_placed": int(self.sensor_indices.numel())
    ...
    def _build_proxy_training_sets(
        ...
        zero_location = (
            tuple(0 for _ in range(len(self._spatial_shape))) if self._spatial_shape else (0, 0)
        )
            ...
            controls.append(
                [
                    self.create_well_control(
                        well_name="dummy", control_type="rate", value=0.0, location=zero_location
                    )
                ]
            )
        ...
        **kwargs,
        ...
        if isinstance(self.proxy_model, LinearDynamicsProxyModel):
            ...
            metrics = self.proxy_model.calibrate(
                historical_states, historical_controls, regularization=regularization
            )
    ...
    def create_reservoir_state(
        ...
        return ReservoirState(
            ...
            saturation=saturation.to(device=self.device, dtype=self.dtype)
    ...
    def create_well_control(
        self, well_name: str, control_type: str, value: float, location: Tuple[int, ...]
    ) -> WellControl:
        ...
        """
        return WellControl(
            well_name=well_name, control_type=control_type, value=value, location=location
        )

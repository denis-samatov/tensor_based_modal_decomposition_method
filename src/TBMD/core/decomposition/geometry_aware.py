...
class GeometryAwareTuckerCore:
        ...
        for iteration in range(self.max_iter):
            ...
            for mode in range(len(tensor.shape)):
                if mode in self.geo_config.spatial_modes:
                    ...
                    # Standard update
                    factors[mode] = self._update_factor_standard(tensor, factors, mode, ranks[mode])
    ...
    def _update_factor_standard(
        self, tensor: torch.Tensor, factors: List[torch.Tensor], mode: int, rank: int
    ) -> torch.Tensor:
    ...
    def _update_factor_regularized(
        self, tensor: torch.Tensor, factors: List[torch.Tensor], mode: int, rank: int
    ) -> torch.Tensor:
        """
        ...
        if self.use_per_mode_laplacians:
            ...
            if not hasattr(self, "_L_torch") or self._L_torch.device != tensor.device:
                ...
                if sp.issparse(L_np):
                    L_np = L_np.toarray()
                self._L_torch = torch.from_numpy(L_np).to(device=tensor.device, dtype=tensor.dtype)
        ...
        U_new = torch.zeros(
            Y_unfolded.shape[0], actual_rank, device=tensor.device, dtype=tensor.dtype
        )
        ...
        for j in range(actual_rank):
            ...
            try:
                ...
                if LTL_dense is not None:
                    ...
                    logger.warning(
                        f"Regularized solve failed for mode column {j}, using unregularized"
                    )
    ...
    def _sparse_scipy_to_torch(
        scipy_sparse: sp.spmatrix, device: torch.device, dtype: torch.dtype
    ) -> torch.Tensor:
...
class GeometryAwareTuckerDecomposer:
    ...
    def __init__(
        ...
        if isinstance(mesh, dict):
            ...
            logger.info(
                f"Built 1D Laplacians for each spatial mode: {list(self.mode_laplacians.keys())}"
            )

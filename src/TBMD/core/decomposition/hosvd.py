...
def DEFAULT_MIDDLE_SLICE_INDEX(shape):
    return shape[2] // 2
...
class ProcessingStrategy(ABC):
    ...
    def process_decomposition(
        self, tensors: Dict[str, torch.Tensor], decomposer_func
...
class CPUStrategy(ProcessingStrategy):
    ...
    def process_reconstruction(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            norm_futures = {
                executor.submit(compute_norm, t): tid for tid, t in unique_tensors.items()
            }
                ...
                if self._is_memory_error(e) and self.fallback_to_cpu:
                    ...
                    try:
                        ...
                        raise TensorDecompositionError(
                            f"Both GPU and CPU decomposition failed: {e}"
                        )
                else:
    ...
    def process_reconstruction(
        ...
        for key in cores:
            try:
                ...
                reconstructed = tucker_to_tensor((cores[key], factors[key]))
                error = float(
                    tl.norm(original_tensors[key] - reconstructed) / tl.norm(original_tensors[key])
                )
                ...
                if self._is_memory_error(e) and self.fallback_to_cpu:
                    ...
                    try:
                        ...
                        logger.error(f"CPU fallback also failed for tensor {key}: {cpu_e}")
                        raise TensorDecompositionError(
                            f"Both GPU and CPU reconstruction failed: {e}"
                        )
                else:
    ...
    def validate_ranks(
        ...
        if isinstance(ranks, int):
            ...
            if ranks > min(tensor_shape):
                raise InvalidRankError(
                    f"Rank {ranks} exceeds minimum tensor dimension {min(tensor_shape)}"
                )
        ...
        if isinstance(ranks, list):
            if len(ranks) != len(tensor_shape):
                raise InvalidRankError(
                    f"Ranks list length {len(ranks)} must match tensor modes {len(tensor_shape)}"
                )

            for i, rank in enumerate(ranks):
                if rank < min_rank:
                    raise InvalidRankError(
                        f"Rank at position {i} must be >= {min_rank}, got {rank}"
                    )
                if rank > tensor_shape[i]:
                    raise InvalidRankError(
                        f"Rank {rank} at position {i} exceeds dimension {tensor_shape[i]}"
                    )
    ...
    def validate_tensor_shape(tensor: torch.Tensor, min_dims: int = 2) -> None:
        ...
        if len(tensor.shape) < min_dims:
            raise ValidationError(
                f"Tensor must have at least {min_dims} dimensions, got {len(tensor.shape)}"
            )
        if any(dim <= 0 for dim in tensor.shape):
            raise ValidationError(
                f"All tensor dimensions must be positive, got shape {tensor.shape}"
            )
...
class TensorProcessor:
    ...
    def process_tensors(
        ...
        tensors: Union[
            ...
            np.ndarray,
            tl.tensor,
            Dict[str, Union[torch.Tensor, np.ndarray, tl.tensor]],
        ],
...
class TuckerDecomposerCore:
    ...
    def decompose_single(self, tensor: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        try:
            return tucker(
                tensor, rank=ranks, init="svd", tol=self.epsilon, random_state=self.random_state
            )
        except Exception as e:
            raise TensorDecompositionError(f"Tucker decomposition failed: {e}")

    def decompose_collection(
        self, tensors: Dict[str, torch.Tensor], strategy: ProcessingStrategy
    ) -> Dict[str, DecompositionResult]:
...
class TensorReconstructor:
    ...
    def reconstruct_single(
        core: torch.Tensor, factors: List[torch.Tensor], original: torch.Tensor
...
class TensorVisualizer:
    ...
    def visualize_single(
        original: torch.Tensor, reconstructed: torch.Tensor, title: str = "Tensor Comparison"
    ) -> None:
    ...
    def visualize_collection(
        ...
        for subject in subjects:
            ...
            TensorVisualizer.visualize_single(
                original_tensors[subject], reconstructed_tensors[subject], f"Subject {subject}"
            )
...
class TuckerDecomposerInterface:
    ...
    def __init__(
        ...
        logger.info(
            f"TuckerDecomposerInterface initialized in {'collection' if self.is_collection else 'single'} mode"
        )
    ...
    def reconstruct(self) -> None:
        ...
        if self.state != DecomposerState.DECOMPOSED:
            raise StateError(
                f"Cannot reconstruct in state {self.state.value}. Call decompose() first."
            )
    ...
    def visualize(self, subjects: Optional[List[str]] = None) -> None:
        ...
        if self.state != DecomposerState.RECONSTRUCTED:
            raise StateError(
                f"Cannot visualize in state {self.state.value}. Call reconstruct() first."
            )

        try:
            if self.is_collection:
                TensorVisualizer.visualize_collection(self.tensors, self._reconstructed, subjects)
            else:

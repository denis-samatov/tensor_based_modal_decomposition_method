...
def generate_navier_stokes_dataset(
    resolution=256, num_samples=20, batch_size=20, record_steps=200, save_path="ns_data.mat"
):
    ...
    for j in range(num_samples // batch_size):
        ...
        t1 = default_timer()
        print(
            f"Batch {j + 1}/{num_samples // batch_size} completed. Generated {c}/{num_samples} samples. Time: {t1 - t0:.2f}s"
        )

    # Save data to file
    print(f"Saving data to {save_path}...")
    scipy.io.savemat(
        save_path, mdict={"a": a.cpu().numpy(), "u": u.cpu().numpy(), "t": sol_t.cpu().numpy()}
    )

...
class MultiResolutionTBMDForecaster:
    ...
    def __init__(self, config: Optional["MultiResolutionTBMDConfig"] = None, **kwargs):
        ...
        if self.config.verbose:
            logger.info(f"MultiResolutionTBMDForecaster initialized ({n_levels} levels)")
            for i, (ranks, ftype) in enumerate(
                zip(self.config.level_ranks, self.config.level_forecaster_types)
            ):
    ...
    def fit(self, X: Union[np.ndarray, torch.Tensor]) -> "MultiResolutionResult":
        ...
        if self.config.verbose:
            logger.info(
                f"Input tensor: ({I1}, {I2}, {T}), T_train={self._T_train}, T_test={self._T_test}"
            )
        ...
        for level_idx in range(len(self.config.level_ranks)):
            ...
            if self.config.verbose:
                ...
                logger.info(
                    f"  Residual norm: {np.linalg.norm(residual):.4f} "
        ...
        if self.config.verbose:
            logger.info(f"\n{'=' * 60}")
            logger.info("Multi-Resolution fit complete!")
            logger.info(
                f"  Total decomposition error: {total_decomp_error:.4f} "
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        for level_idx, level in enumerate(self._levels):
            ...
            latent_r2 = 1.0 - (
                np.sum((C_target - C_pred[:n_level_eval]) ** 2)
        ...
        if self.config.verbose:
            ...
            for plm in per_level_metrics:
                logger.info(
                    f"  Level {plm['level']}: ranks={plm['ranks']}, "
    ...
    def plot_spatial_comparison(
        ...
        if time_idx >= len(self._eval_target):
            raise ValueError(
                f"time_idx {time_idx} out of range (max: {len(self._eval_target) - 1})"
            )

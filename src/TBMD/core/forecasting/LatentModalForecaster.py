...
class LatentModalForecaster:
    ...
    def __init__(self, config: Optional["LatentModalForecasterConfig"] = None, **kwargs):
        ...
        if self.config.verbose:
            logger.info(
                f"LatentModalForecaster initialized (device={self._device}, "
                f"forecaster_type={self.config.forecaster_type})"
            )

    def fit(
        self, X: Union[np.ndarray, torch.Tensor], ranks: Optional[Union[int, List[int]]] = None
    ) -> "LatentModalResult":
        """
        ...
        if self._T_train < 3:
            raise ValueError(
                f"Not enough training time steps: {self._T_train}. "
                f"Increase train_ratio or provide more data."
            )
        if self._T_test < 1:
            raise ValueError(f"Not enough test time steps: {self._T_test}. Decrease train_ratio.")
    ...
    def _decompose_training_data(
        self, X_train: np.ndarray, ranks: Optional[Union[int, List[int]]]
    ) -> Tuple:
        """
        ...
        if ranks is None:
            ...
            validated_ranks = [min(ranks[0], I1), min(ranks[1], I2), min(ranks[2], T_train)]
        else:
    ...
    def _project_to_latent(
        self, X_slice: np.ndarray, A: np.ndarray, B: np.ndarray, G: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _project_to_latent_batch(
        self, X: np.ndarray, A: np.ndarray, B: np.ndarray, G: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _reconstruct_from_latent(self, c: np.ndarray) -> np.ndarray:
        ...
        if (
            getattr(self.config, "latent_normalization", False)
            and hasattr(self, "_latent_mean")
            and self._latent_mean is not None
        ):
        ...
        if (
            getattr(self.config, "spatial_mean_centering", False)
            and hasattr(self, "_spatial_mean")
            and self._spatial_mean is not None
        ):
    ...
    def _train_sub_forecaster(self, C_train: np.ndarray, R3: int) -> None:
        ...
        if self.config.verbose:
            logger.info(
                f"Training {forecaster_type.upper()} sub-forecaster on latent space (dim={R3})..."
            )

        if forecaster_type == "linear":
            self._sub_forecaster = LinearForecaster(
                config=LinearForecasterConfig(device=self._device, verbose=self.config.verbose)
            )
    ...
    def predict_sequence_latent(self, c_start: np.ndarray, n_steps: int) -> np.ndarray:
        """
    ...
    def predict_sequence_spatial(
        self, c_start: np.ndarray, n_steps: int
    ...
    def evaluate(self, X_full: Optional[Union[np.ndarray, torch.Tensor]] = None) -> Dict[str, Any]:
        ...
        spatial_rel_frob = np.linalg.norm(X_target_flat - X_pred_flat) / np.linalg.norm(
            X_target_flat
        )

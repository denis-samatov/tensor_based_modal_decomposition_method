...
class ReservoirState:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            'pressure': self.pressure.cpu().numpy() if isinstance(self.pressure, torch.Tensor) else self.pressure,
            'saturation': self.saturation.cpu().numpy() if self.saturation is not None and isinstance(self.saturation, torch.Tensor) else self.saturation,
            'time': self.time,
...
class ReservoirProxyModelBase(ABC):
    ...
    def forecast(self, 
                 ...
                 time_horizon: float,
                 time_steps: int) -> List[ReservoirState]:
        """
...
class LinearDynamicsProxyModel(ReservoirProxyModelBase):
    ...
    def calibrate(self, 
                  ...
                  regularization: float = 1e-4) -> Dict[str, float]:
        """
        ...
        if n_wells > 0:
            control_inputs = torch.zeros((len(historical_controls), n_wells), 
        ...
        if control_inputs is not None:
            ...
            reg_term = regularization * torch.eye(self.n_modes, device=self.device, dtype=self.dtype)
    ...
    def forecast(self,
        ...
        if self.B is not None and well_controls:
            u = torch.tensor([ctrl.value for ctrl in well_controls],
                           device=self.device, dtype=self.dtype)
        else:
            u = None
...
class PhysicsInformedProxyModel(LinearDynamicsProxyModel):
    ...
    def enforce_mass_conservation(self, 
                                  ...
                                  well_controls: List[WellControl]) -> torch.Tensor:
        """
        ...
        if abs(total_injection - total_production) > 1e-6:
            logger.warning(f"Mass imbalance: injection={total_injection:.2f}, "
    ...
    def forecast(self,
        ...
        forecasted_states = super().forecast(current_state, well_controls, 

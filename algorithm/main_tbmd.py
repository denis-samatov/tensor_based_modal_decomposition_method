...
# subject_name = next(k for i, k in enumerate(tensors['pressure'].keys()) if i == 2)
...
def make_linear_solver(cfg: ExtensionCompressiveSensingConfig) -> LinearSolver:
    ...
    def get_eye(n: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        key = (n, device, dtype)
        if key not in eye_cache:
            eye_cache[key] = reg * torch.eye(n, device=device, dtype=dtype)
        return eye_cache[key]

    def cholesky(lhs: torch.Tensor, rhs: torch.Tensor) -> torch.Tensor:
        lhs_reg = lhs + get_eye(lhs.shape[0], lhs.device, lhs.dtype)
        try:
...
class ExperimentRunner:
    ...
    def run_single_slice_wells_experiments(self,
        ...
        valid_wells = [[i, j] for i, j in dict.fromkeys((i, j) for i, j in wells_list if 0 <= i < A_tensor.shape[0] and 0 <= j < A_tensor.shape[1])]

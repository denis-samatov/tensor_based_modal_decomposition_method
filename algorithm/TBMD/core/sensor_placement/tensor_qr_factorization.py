...
class UniformDistributionManager:
    ...
    def __init__(self, config: SensorPlacementConfig, spatial_shape: Tuple[int, ...], device: torch.device):
        self.config = config
...
class TensorTubeQRDecomposition:
    ...
    def __init__(
        ...
        effective_random_state = random_state if random_state is not None else (self.config.random_state or self.config.seed)
        ...
        effective_dtype = dtype if dtype is not None else (
        ...
        self.uniform_distribution = uniform_distribution if uniform_distribution is not None else self.config.uniform_distribution
        self.check_orthogonality = check_orthogonality if check_orthogonality is not None else self.config.check_orthogonality

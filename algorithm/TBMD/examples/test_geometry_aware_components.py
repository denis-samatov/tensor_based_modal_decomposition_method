...
class TestMeshGraphBuilder:
    ...
    def test_laplacian_properties(self):
        ...
        print(f"✓ Laplacian properties validated")
...
class TestGeometryAwareHOSVD:
    ...
    def test_basic_decomposition(self):
        ...
        error = torch.norm(torch.from_numpy(tensor) - reconstructed) / torch.norm(torch.from_numpy(tensor))
...
class TestGeometryAwareQR:
    ...
    def test_gradient_priority(self):
        ...
        if len(sensor_positions) > 0:
            ...
            print(f"✓ Gradient priority: {inner_sensors}/{len(sensor_positions)} sensors in inner region")
...
def visualize_mesh_and_sensors():
    ...
    axes[2].scatter(sensor_pos[:, 1], sensor_pos[:, 0], 

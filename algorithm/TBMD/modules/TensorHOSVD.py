...
# class IterativeHOSVDDecomposer:
#     """
#     Implementation of the iterative HOSVD algorithm as described in Algorithm 1.
    
#     This implements the true HOSVD algorithm with convergence criteria,
#     not just the standard Tucker decomposition.
#     """
    
#     def __init__(self, epsilon: float = DEFAULT_EPSILON, 
#                  max_iterations: int = 100,
#                  rank_reduction_strategy: str = 'decrement',
#                  random_state: Optional[int] = None):
#         """
...
#             rank_reduction_strategy: How to reduce ranks ('decrement' or 'fixed')
#             random_state: Random seed for reproducibility
#         """
#         self.epsilon = TensorValidator.validate_epsilon(epsilon)
#         self.max_iterations = max_iterations
#         self.rank_reduction_strategy = rank_reduction_strategy
#         self.random_state = random_state
#         logger.info(f"IterativeHOSVDDecomposer initialized with epsilon: {self.epsilon}")
    
#     def _compute_tensor_norm(self, tensor: torch.Tensor) -> float:
#         """Compute Frobenius norm of tensor"""
#         return float(torch.norm(tensor.flatten()))
    
#     def _get_mode_unfolding(self, tensor: torch.Tensor, mode: int) -> torch.Tensor:
#         """
#         Compute mode-n unfolding of tensor X_(n)
        
#         Args:
#             tensor: Input tensor of shape (I1, I2, ..., IN)
#             mode: Mode along which to unfold (0-indexed)
            
#         Returns:
#             Matrix of shape (In, I1*I2*...*I_{n-1}*I_{n+1}*...*IN)
#         """
...
#         # Reshape to matrix: mode dimension x all other dimensions
#         mode_size = unfolded.shape[0]
#         other_size = unfolded.numel() // mode_size
        
#         return unfolded.reshape(mode_size, other_size)
    
#     def _compute_leading_singular_vectors(self, matrix: torch.Tensor, 
#                                         num_vectors: int) -> torch.Tensor:
#         """
#         Compute the leading left singular vectors of a matrix.
        
#         Args:
#             matrix: Input matrix
#             num_vectors: Number of leading singular vectors to compute
            
#         Returns:
#             Matrix with leading left singular vectors as columns
#         """
#         try:
#             # Perform SVD
#             U, S, Vh = torch.linalg.svd(matrix, full_matrices=False)
            
#             # Take only the required number of vectors
#             num_vectors = min(num_vectors, U.shape[1])
#             leading_vectors = U[:, :num_vectors]
            
#             return leading_vectors
            
#         except Exception as e:
#             raise TensorDecompositionError(f"SVD computation failed: {e}")
    
#     def _tensor_mode_product(self, tensor: torch.Tensor, matrix: torch.Tensor, 
#                            mode: int) -> torch.Tensor:
#         """
#         Compute tensor-matrix product along specified mode.
        
#         This computes X ×_n U^T where ×_n is the mode-n product.
        
#         Args:
#             tensor: Input tensor
#             matrix: Matrix to multiply (will be transposed)
#             mode: Mode along which to perform the product
            
#         Returns:
#             Resulting tensor after mode-n product
#         """
#         # Get tensor dimensions
#         tensor_shape = list(tensor.shape)
        
#         # Move the mode to the front for easier manipulation
#         dims = list(range(tensor.ndim))
#         dims[0], dims[mode] = dims[mode], dims[0]
#         tensor_reordered = tensor.permute(dims)
        
#         # Reshape for matrix multiplication
#         mode_size = tensor_reordered.shape[0]
#         other_dims = tensor_reordered.shape[1:]
#         tensor_matrix = tensor_reordered.reshape(mode_size, -1)
        
#         # Multiply with transposed matrix
#         result_matrix = torch.mm(matrix.T, tensor_matrix)
        
#         # Reshape back to tensor
#         new_mode_size = result_matrix.shape[0]
#         result_tensor = result_matrix.reshape(new_mode_size, *other_dims)
        
#         # Permute back to original mode order
#         result_tensor = result_tensor.permute(dims)  # This undoes the initial permutation
        
#         return result_tensor
    
#     def decompose_iterative_hosvd(self, tensor: torch.Tensor, 
#                                  initial_ranks: Optional[List[int]] = None) -> Tuple[torch.Tensor, List[torch.Tensor]]:
#         """
#         Perform iterative HOSVD decomposition according to Algorithm 1.
        
#         Algorithm 1: 3rd-order Tensor HOSVD Algorithm
#         INPUT: X, ε
#         OUTPUT: G, A, B, C
        
#         Args:
#             tensor: Input tensor X
#             initial_ranks: Initial rank estimates R_n^(0) for each mode
            
#         Returns:
#             Tuple of (core_tensor G, [factor_matrices U^(1), U^(2), U^(3)])
#         """
#         TensorValidator.validate_tensor_shape(tensor, min_dims=3)
        
#         if tensor.ndim != 3:
#             raise ValidationError("Iterative HOSVD currently supports only 3rd-order tensors")
        
#         # Algorithm initialization
#         k = 0  # iteration counter
#         G_prev = torch.zeros_like(tensor)  # G^(0) ← 0
        
#         # Initialize ranks R_n^(0) ← min(i,j,k)  
#         if initial_ranks is None:
#             min_dim = min(tensor.shape)
#             R_current = [min_dim] * 3
#         else:
#             R_current = TensorValidator.validate_ranks(initial_ranks, tensor.shape)
        
#         # Compute initial residual r ← ||X||_F
#         r = self._compute_tensor_norm(tensor)
#         X_norm = r  # Store original norm for relative error
        
#         logger.info(f"Starting iterative HOSVD: initial residual = {r:.6f}")
#         logger.info(f"Initial ranks: {R_current}")
        
#         # Storage for factor matrices
#         U_matrices = [None] * 3
        
#         # Main loop: while r ≥ ε do
#         iteration_count = 0
#         convergence_history = []
        
#         while r >= self.epsilon and iteration_count < self.max_iterations:
#             logger.debug(f"Iteration {k+1}: residual = {r:.6f}, ranks = {R_current}")
            
#             # For n = 1,2,3 do
#             for n in range(3):
#                 # Compute mode-n unfolding X_(n)
#                 X_n = self._get_mode_unfolding(tensor, n)
                
#                 # U^(n) ← R_n^(k) leading left singular vectors of X_(n)
#                 U_n = self._compute_leading_singular_vectors(X_n, R_current[n])
#                 U_matrices[n] = U_n
                
#                 # R_n^(k+1) ← R_n^(k) - 1 (rank reduction strategy)
#                 if self.rank_reduction_strategy == 'decrement':
#                     R_current[n] = max(1, R_current[n] - 1)
#                 # For 'fixed' strategy, ranks remain unchanged
            
#             # k ← k + 1
#             k += 1
#             iteration_count += 1
            
#             # G^k ← X ×_1 U^(1)T ×_2 U^(2)T ×_3 U^(3)T
#             G_current = tensor
#             for n in range(3):
#                 G_current = self._tensor_mode_product(G_current, U_matrices[n], n)
            
#             # r ← ||G^k - G^(k-1)||_F
#             G_diff = G_current - G_prev
#             r = self._compute_tensor_norm(G_diff)
            
#             # Store convergence info
#             relative_error = r / X_norm if X_norm > 0 else r
#             convergence_history.append({
#                 'iteration': k,
#                 'absolute_residual': r,
#                 'relative_residual': relative_error,
#                 'ranks': R_current.copy()
#             })
            
#             # Update for next iteration
#             G_prev = G_current.clone()
            
#             # Early stopping check
#             if relative_error < 1e-12:
#                 logger.info(f"Early stopping: relative error {relative_error:.2e} < 1e-12")
#                 break
        
#         # Final results: G ← G^k, Â ← U^(1), B̂ ← U^(2), Ĉ ← U^(3)
#         final_core = G_current
#         final_factors = U_matrices
        
#         # Log convergence information
#         if r < self.epsilon:
#             logger.info(f"Converged after {k} iterations: residual = {r:.6f} < {self.epsilon}")
#         else:
#             logger.warning(f"Did not converge after {k} iterations: residual = {r:.6f} >= {self.epsilon}")
        
#         logger.info(f"Final ranks: {[U.shape[1] for U in U_matrices]}")
        
#         # Store convergence history for analysis
#         self.convergence_history = convergence_history
#         self.final_residual = r
#         self.iterations_used = k
        
#         return final_core, final_factors
    
#     def get_convergence_info(self) -> Dict:
#         """Get detailed convergence information"""
#         if not hasattr(self, 'convergence_history'):
#             raise StateError("No decomposition has been performed yet")
        
#         return {
#             'convergence_history': self.convergence_history,
#             'final_residual': self.final_residual,
#             'iterations_used': self.iterations_used,
#             'converged': self.final_residual < self.epsilon
#         }
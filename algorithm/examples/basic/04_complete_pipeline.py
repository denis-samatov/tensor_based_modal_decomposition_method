...
def visualize_results(data, results, args):
    ...
    for j in range(3):
        ...
        if len(var_sensors) > 0:
            ax.scatter(var_sensors.numpy(), 
                      data[var_sensors, j, t_sample].numpy(),
                      c='red', s=50, zorder=5)
    ...
    ax.axhline(y=recon['mean_error'], color='red', linestyle='--', 
              label=f"Mean: {recon['mean_error']:.4f}")
    ...
    for idx, (data_slice, title) in enumerate([
        ...
        (torch.abs((data[:, var_idx, :] - recon['data'][:, var_idx, :]) / (data[:, var_idx, :] + 1e-8)), 'Relative Error')
    ]):
        ax = fig.add_subplot(gs[3, idx])
        im = ax.imshow(data_slice.numpy(), aspect='auto', 
    ...
    plt.suptitle(f'TBMD Complete Pipeline Results\n'

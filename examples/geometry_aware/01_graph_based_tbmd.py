...
def compare_standard_vs_geometry_aware(
    ...
    standard_decomposer = TuckerDecomposer(
        ...
        ranks=[n_modes, n_modes, n_modes // 2],  # 3 ranks for a 3D tensor (spatial_x, spatial_y, time).
        device='cpu'
    )
...
def visualize_spatial_modes_comparison(
    ...
    plt.suptitle('Spatial Mode Comparison: Standard vs Geometry-Aware',
...
def test_reconstruction_with_geometry(
    ...
    print('-' * 85)
    print(f"{'Standard (standard CS)':<45} {metrics_std['rmse']:.6f}  {metrics_std['ssim']:.6f}  {metrics_std['relative_error']:.6f}")
...
def visualize_reconstruction_comparison(
    ...
    ax2.set_title(f'Standard (SSIM={metrics_std["ssim"]:.4f})',
    ...
    ax5.set_title(f'Error (RMSE={metrics_std["rmse"]:.4f})',
                 fontsize=12, fontweight='bold')
    ax5.axis('off')
    ...
    im6 = ax6.imshow(error_geo, cmap='Reds', aspect='auto', origin='lower',
                     vmin=0, vmax=error_max)
    ax6.set_title(f'Error (RMSE={metrics_geo["rmse"]:.4f})',
                 fontsize=12, fontweight='bold')
    ax6.axis('off')
    plt.colorbar(im6, ax=ax6, fraction=0.046)
    
    plt.suptitle('Reconstruction Comparison: Standard vs Geometry-Aware',
...
def test_adaptive_alpha(
    ...
    print('-' * 68)
    print(f"{'Fixed alpha=0.05':<30} {metrics_fixed['rmse']:.6f}  {metrics_fixed['ssim']:.6f}  {reconstructor_fixed.alpha:.6f}")
    print(f"{'Adaptive alpha':<30} {metrics_adapt['rmse']:.6f}  {metrics_adapt['ssim']:.6f}  {reconstructor_adapt.alpha:.6f}")
    print('-' * 68)

...
def generate_test_problem(n_spatial: int = 100, n_modes: int = 20, 
                         sensor_fraction: float = 0.3, noise_level: float = 0.01):
    """
    ...
    xx, yy = np.meshgrid(
        np.linspace(0, 2*np.pi, n_spatial),
        np.linspace(0, 2*np.pi, n_spatial)
    )
...
def compare_reconstructions(A, P, Y, x_true, spatial_shape, mesh,
    ...
    config_std = CompressiveSensingConfig(
        max_iter=500,
        epsilon_l1=1e-2,
        tol=1e-4,
        device='cpu'
...
def visualize_results(A, x_true, results, spatial_shape, save_path=None):
    ...
    for i, (method_name, result) in enumerate(results.items(), 1):
        ...
        # Coefficients
        axes[1, i].stem(x_rec, basefmt=' ', linefmt='b-', markerfmt='bo', 
                       label='Recovered')
        axes[1, i].stem(x_true, basefmt=' ', linefmt='k:', markerfmt='k^',
                       label='True', alpha=0.5)
        axes[1, i].set_title(f'Recovered (error: {np.linalg.norm(x_rec - x_true):.3f})',
                            fontsize=10)
...
def main():
    ...
    results = compare_reconstructions(
        A, P, Y, x_true, spatial_shape, mesh,
    ...
    visualize_results(
        A, x_true, results, spatial_shape,
        save_path=output_dir / 'geometry_aware_cs_comparison.png'
    )
    
    plot_convergence(
        results,
        save_path=output_dir / 'geometry_aware_cs_convergence.png'
    )

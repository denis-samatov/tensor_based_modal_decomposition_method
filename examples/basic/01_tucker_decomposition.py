...
for rank in ranks_to_test:
    ...
    print(f"   Rank={rank}: error={decomposer.reconstruction_errors:.4f}")
...
compression_ratio = (I * J * T) / (
    best_result.factors[0].numel() + best_result.factors[1].numel() + best_result.cores.numel()
)
...
print(
    f"   Compressed size: {best_result.factors[0].numel() + best_result.factors[1].numel() + best_result.cores.numel()} elements"
)
...
try:
    ...
    axes[0, 0].plot(
        reconstructed[50, 0, :].numpy(), label="Reconstructed", linestyle="--", linewidth=2
    )
    ...
    for i in range(n_modes_to_show):
        ...
        if best_result.factors[1].shape[0] == T:
            axes[1, 1].plot(best_result.factors[1][:, i].numpy(), label=f"Mode {i + 1}", alpha=0.7)
        else:
            axes[1, 1].plot(best_result.factors[1][i, :].numpy(), label=f"Mode {i + 1}", alpha=0.7)

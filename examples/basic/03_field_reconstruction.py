...
decomp_config = DecompositionConfig(ranks=[15, 10], verbose=False)
...
# 3. Sensor placement.
print("\n3. Sensor placement...")
sensor_config = SensorPlacementConfig(n_sensors=30, verbose=False)
...
recon_config_ls = ReconstructionConfig(solver="least_squares", verbose=False)
...
recon_config_admm = ReconstructionConfig(
    solver="admm", max_iterations=100, lambda_reg=0.01, verbose=False
)
...
recon_config_ista = ReconstructionConfig(
    solver="ista", max_iterations=200, lambda_reg=0.01, verbose=False
)
...
try:
    ...
    ax1.scatter(
        sensor_result.sensor_indices[sensor_result.sensor_indices < I],
    ...
    for idx, (field, title) in enumerate(
        [
            (test_field, "Original"),
            (reconstructed_admm, "Reconstructed (ADMM)"),
    ...
    for var in range(J):
        ...
        ax.plot(
            reconstructed_sequence[point_idx, var, :].numpy(),
            label="Reconstructed",
            linestyle="--",
            linewidth=2,
        )

...
def create_synthetic_decomposition_data(
    spatial_dims: tuple = (50, 50), time_steps: int = 100, ranks: tuple = (10, 10, 15)
) -> tuple:
    """
    ...
    """
    logger.info(
        f"Creating synthetic data: spatial {spatial_dims}, time {time_steps}, ranks {ranks}"
    )
    # Create core tensor
...
def demonstrate_processing_strategies():
    ...
    for strategy in strategies:
        ...
        end_time = time.time()
        results[strategy] = {"result": result, "time": end_time - start_time, "shape": result.shape}
...
def demonstrate_batch_processing():
    ...
    for i in range(5):
        ...
        core, factors = create_synthetic_decomposition_data(
            spatial_dims=(40, 35), time_steps=time_steps, ranks=(8, 10, 12)
        )
...
def demonstrate_error_handling():
    ...
    for test_case in test_cases:
        ...
        try:
            processor.process_single_subject(test_case["core"], test_case["factors"])
...
def demonstrate_performance_analysis():
    ...
    config = ModalProcessorConfig(processing_strategy=ProcessingStrategy.BATCH, batch_size=50)
    processor = ModalTensorProcessor(config)
    ...
    for test_config in test_configs:
        ...
        start_time = time.time()
        processor.process_single_subject(core, factors)
        end_time = time.time()

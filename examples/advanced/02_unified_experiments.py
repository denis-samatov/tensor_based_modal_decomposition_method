...
def demo_auto_detection():
    ...
    config2 = ExperimentConfig(
        device="cpu", noise_level=0.1, num_noise_samples=3, noise_threshold=1e-10
    )
...
def demo_explicit_control():
    ...
    df_simple = runner.run_experiments(
        A_tensor, test_tensors, sensor_values, statistical_analysis=False
    )
    print(f"Columns: {list(df_simple.columns)}")

    # Force statistical mode.
    print("\nForce statistical mode (statistical_analysis=True):")
    df_stat = runner.run_experiments(
        A_tensor, test_tensors, sensor_values, statistical_analysis=True
    )
...
def demo_performance_comparison():
    ...
    df_simple = runner.run_experiments(
        A_tensor, test_tensors, sensor_values[:2], statistical_analysis=False
    )
    ...
    start_time = time.time()
    df_stat = runner.run_experiments(
        A_tensor, test_tensors, sensor_values[:2], statistical_analysis=True
    )
...
def demo_plotting_compatibility():
    ...
    plot_analytics(
        df_stat, plot_type="normalized", title_prefix="Statistical Mode", show_plots=False
    )

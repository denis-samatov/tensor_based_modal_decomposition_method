...
def demonstrate_advanced_configuration():
    ...
    print(f"Advanced factorization results:")
    ...
    print(f"  Max orthogonality deviation during factorization: {info.get('max_orthogonality_deviation', 'N/A'):.2e}")
...
def demonstrate_numerical_stability():
    ...
    config = TensorQRConfig(
        MACHINE_EPSILON_FACTOR=1e-16,
        HOUSEHOLDER_THRESHOLD=1e-16,
    ...
    try:
        ...
        print(f"Numerical stability test:")
...
def demonstrate_error_handling():
    ...
    test_cases = [
        ...
        ("Infinite tensor", lambda: torch.tensor([[[1.0, float('inf')]]]), "Infinite values detected"),
    ...
    for test_name, tensor_func, expected_error in test_cases:
        try:
            if test_name == "Invalid sensor count":
                ...
                decomposer = TensorTubeQRDecomposition(tensor, N)
            else:
                tensor = tensor_func()
                decomposer = TensorTubeQRDecomposition(tensor, N=2)

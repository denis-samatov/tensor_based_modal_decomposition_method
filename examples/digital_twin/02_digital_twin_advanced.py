...
def generate_synthetic_reservoir_data(
    spatial_shape=(50, 50),
    n_timesteps=100,
    n_wells=5,
    seed=42
    ...
    for t in range(n_timesteps):
        ...
        field = (base_pressure * time_factor +
        ...
        for i, (wx, wy) in enumerate(well_locations):
            ...
            well_controls.append(
                WellControl(f'WELL_{i+1}', 'rate', rate, (wx, wy))
            )
...
def plot_pressure_field(field, title="Pressure Field", wells=None, sensors=None):
    ...
    if wells is not None:
        ...
        ax.scatter(well_x, well_y, c='red', marker='s', s=100, 
                  label='Wells', edgecolors='black', linewidths=2)
    ...
    if sensors is not None:
        ...
        if len(sensor_positions) > 0:
            ax.scatter(sensor_positions[:, 1], sensor_positions[:, 0], 
                      c='cyan', marker='o', s=50, label='Sensors',
...
def plot_monitoring_history(twin):
    ...
    ax1.axhline(y=twin.config.alert_threshold, color='orange', 
               linestyle='--', label='Warning Threshold')
    ax1.axhline(y=2*twin.config.alert_threshold, color='red', 
...
def main():
    ...
    # Train the twin
    training_summary = twin.train(
        historical_data=train_data,
        historical_controls=train_controls
    )
    
    print(f"\n  Training Summary:")
    print(f"    Reconstruction error: {training_summary['decomposition']['reconstruction_error']:.4f}")
    ...
    for t_idx in range(len(monitor_data.shape[-1])):
        ...
        # Predict next state
        predicted_states = twin.predict_next_state(
        ...
        if t_idx % 5 == 0:
            print(f"  t={current_time:.1f}: error={update_result['metrics'].get('relative_error', 0):.4f}, "
    ...
    # Baseline scenario
    scenarios['Baseline'] = [
        WellControl(f'WELL_{i+1}', 'rate', 
                   -5.0 if i % 2 == 0 else 8.0, 
                   well_locations[i])
    ...
    fig3 = plot_scenario_comparison(scenario_results, kpi='avg_pressure')
    ...
    print(f"  Alerts - Normal: {summary['normal_count']}, "

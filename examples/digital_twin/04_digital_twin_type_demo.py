...
def generate_synthetic_data(spatial_shape=(20, 20), n_time_steps=50):
    ...
    for t in range(n_time_steps - 1):
        ...
        # Laplacian
        laplacian = (
            torch.roll(current, 1, 0)
            + torch.roll(current, -1, 0)
            + torch.roll(current, 1, 1)
            + torch.roll(current, -1, 1)
            - 4 * current
        )
...
def run_demo():
    ...
    if config.proxy_model_type:
        dummy_states = [
            twin.create_reservoir_state(p, time=float(i))
            for i, p in enumerate(train_data.unbind(-1))
        ]
    ...
    for t in range(test_data.shape[-1]):
        ...
        predicted_states = twin.predict_with_controls(
            twin.state.reservoir_state or current_state, next_ctrls, time_horizon=1.0, time_steps=1
        )
        ...
        twin.update_from_sensors(sensor_readings, timestamp=n_train + t)

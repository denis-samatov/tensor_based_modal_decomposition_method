...
def main():
    ...
    print(
        f"Splitting data: {train_steps} steps for training, {n_time - train_steps} for simulation."
    )
    ...
    train_tensors_processed = process_data(
        train_dict, normalization_method="minmax", global_params=minmax_params
    )

    test_tensors_processed = process_data(
        test_dict, normalization_method="minmax", global_params=minmax_params
    )
